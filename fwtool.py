...
def writeYaml(yamlData, file):
    ...
    yaml.add_representer(dict, lambda dumper, data: dumper.represent_mapping(dumper.DEFAULT_MAPPING_TAG, data,
                                                                             flow_style=False))
...
def main():
    ...
    packConfig.add_argument('-c', dest='configFile', type=argparse.FileType('rb'),
    ...
    packBody.add_argument('-b', dest='updaterBodyFile', type=argparse.FileType('rb'),
                          help='updater body file (libupdaterbody.so)')
    ...
    if args.command == 'unpack':
        ...
        packCommand(args.firmwareFile, args.updaterFile, args.updaterBodyFile, args.configFile, args.device,
                    args.outDir)
