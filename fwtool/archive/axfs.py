...
def readAxfs(file):
    ...
    for i, k in enumerate(axfsRegions):
        ...
        if i >= 4:
            ...
            tables[k] = [sum([ord(regionData[j * region.maxIndex + i:j * region.maxIndex + i + 1]) << (8 * j) for j in
                              range(region.tableByteDepth)]) for i in range(region.maxIndex)]
        ...
        def generateChunks(arrayIndex=arrayIndex, numEntries=numEntries, size=size):
            ...
            for i in range(numEntries):
                ...
                if nodeType == 0:
                    ...
                    contents = zlib.decompress(regions['compressed'].read(
                        tables['cblockOffset'][cnodeIndex + 1] - tables['cblockOffset'][cnodeIndex]))
                elif nodeType == 2:
